...
// x86-lite Assembler v4.3 (robust parsing, label resolution, clear errors)
...
function operandFrom(tok){
  ...
  // treat as label reference (resolved in pass 2)
  return {kind:'label', name: tok};
}

function enc_operand(op){
  if (op.kind==='reg') return [0, [op.reg]];
  if (op.kind==='imm') return [1, [op.val & 0xFF]]; // 8-bit data path in this VM
  if (op.kind==='mem'){
    const s = op.spec;
...
function assemble(source){
  ...
  for (let lineNo=0; lineNo<lines.length; lineNo++){
    ...
    if (/:\s*$/.test(s)){ // label
      ...
      if (!name) throw new Error(`Leeres Label in Zeile ${lineNo+1}`);
      labels.set(name, pc);
      continue;
    }
    ...
    // For size calc, replace label operands with imm16 placeholder
    const opsForSize = ops.map(op => op.kind==='label' ? {kind:'imm', val:0} : op);

    let size = 0;
    switch(mnem){
      case 'HLT': case 'RET': size = 1; break;
      case 'JMP': case 'JZ': case 'JNZ': case 'CALL': size = 1 + 2; break;
      ...
      case 'MOV': case 'ADD': case 'SUB': case 'CMP': case 'AND': case 'OR': case 'XOR':
        size = size_twoop(opsForSize[0], opsForSize[1]); break;
      default: throw new Error(`Unbekannter Mnemonic "${mnem}" (Zeile ${lineNo+1})`);
    }
  ...
  // Pass 2: emit with label resolution
  const out = [];
  function outb(b){ out.push(b & 0xFF); }
  function out16(n){ outb(n & 0xFF); outb((n>>8)&0xFF); }

  for (const t of toks){
    const {mnem, ops, lineNo} = t;

    const resolve = (op) => {
      if (op.kind==='label'){
        if (!labels.has(op.name)) throw new Error(`Unbekanntes Label "${op.name}" (Zeile ${lineNo+1})`);
    ...
    switch(mnem){
      ...
      case 'JMP': case 'JZ': case 'JNZ': case 'CALL': {
        ...
        if (o.kind!=='imm') throw new Error(`${mnem} benötigt eine absolute Adresse oder ein Label (Zeile ${lineNo+1})`);
      ...
      default:
        throw new Error(`Nicht implementiert: ${mnem}`);
    }
  }
